...
namespace serial_connector_ros2
{
    ...
    {
        public:
        SerialHandler(std::string port_name="/dev/ttyACM0", int baud_rate=115200):port_name_(port_name),baud_rate_(baud_rate){}

        static SerialHandler* init_handler(std::string port_name, int baud_rate)
        {
            return (SerialHandler *)(new SerialHandler(port_name, baud_rate));
        }
